...
├── workflows/                      # n8n 工作流导出（通过 n8n API 导出，已版本控制）
│   ├── 01-perspective-single.json      # ✅ 已导出
│   ├── 02-perspective-committee.json   # ✅ 已导出
│   ├── 03-research-to-article.json     # ✅ 已导出
│   └── 04-feishu-bot.json              # ⏳ Stage 4 时生成
└── docs/
...
| **Stage 1** | 单 perspective 问答（Form 表单 + 多模型切换） | Trigger / Expression / Credentials / HTTP Request / Switch / Merge | ✅ 完成（在 n8n active，已导出 `workflows/01-perspective-single.json`） |
| **Stage 2** | 多 perspective 委员会（并行 5 路 + 综合） | Code / Loop / Aggregate / 并行执行 | ✅ 完成（在 n8n active，已导出 `workflows/02-perspective-committee.json`） |
| **Stage 3** | 研究 → 写作流水线（Tavily 搜索 + hv-analysis + khazix-writer + 发邮件） | 多步链 / IF / Tavily / Email | ✅ 完成（在 n8n active，已导出 `workflows/03-research-to-article.json`） |
| **Stage 4** | 飞书 bot（@bot + 问题 → 在线程里回复） | Webhook / Execute Workflow / OAuth2 | ⏳ 下一步 |

> **进度同步说明（2026-05-30）**：Stage 1/2/3 的工作流均已在 Elestio 上的 n8n 实例里搭好并处于 active 状态，并通过 n8n API 导出备份到本仓库 `workflows/` 目录做版本控制。后续在 n8n UI 改动工作流后，记得重新导出并 commit，保持仓库与线上一致。
...
1. **（可选）打磨现有工作流**：测试 Stage 1/2/3 端到端跑通，补齐其余 5 家 provider 的 credential
2. **Stage 4：飞书 bot** —— Webhook 接收飞书 @bot 事件 → Execute Workflow 复用 Stage 1/2 → 在线程里回复
3. 在 n8n UI 改动任何工作流后，重新导出并 commit，保持仓库与线上同步：
   ```bash
   # 通过 n8n API 导出（或在 UI 里 ⋯ → Download）
   git add workflows/ && git commit -m "chore: 同步 n8n 工作流" && git push
   ```
